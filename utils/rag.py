...
def print_text_to_image_citation_single(imageno, image_dict, color=None):
    if color is None:
        color = Color()
...
def print_text_to_image_citation(
    ...
    for imageno, image_dict in final_images.items():
        print_text_to_image_citation_single(imageno, image_dict, color)
...
def get_similar_image_from_query(
    ...
    Returns:
        A dictionary containing information about the top N most similar images, including cosine scores, image objects,
        paths, page numbers, text excerpts, and descriptions.
    """
...
def create_images(
        images: Iterable[Union[str, PilImage.Image]], resize_ratio: float = 0.5
) -> None:
    """
    ...
    for image in images:
        if isinstance(image, str):
            pil_images.append(PilImage.open(image))
        else:
